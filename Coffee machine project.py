MENU = {
    "espresso": {
        "ingredients": {
            "water": 50,
            "coffee": 18,
        },
        "cost": 1.5,
    },
    "latte": {
        "ingredients": {
            "water": 200,
            "milk": 150,
            "coffee": 24,
        },
        "cost": 2.5,
    },
    "cappuccino": {
        "ingredients": {
            "water": 250,
            "milk": 100,
            "coffee": 24,
        },
        "cost": 3.0,
    }
}

profit = 0
resources = {
    "water": 300,
    "milk": 200,
    "coffee": 100,
}

# Todo 1 print a report of all coffee machine resources
def resource_availability(order_ingredients):
    """Returns True when order can be made, False if ingredients are insufficient."""
    for item in order_ingredients:
        if order_ingredients[item] > resources[item]:
            print(f"Sorry there is not enough {item}.")
            return False
        return True

def payment():
    """Returns the total calculated from coins inserted."""
    print("Please insert coins.")
    total = int(input("how many quarters?: ")) * 0.25
    total += int(input("how many dimes?: ")) * 0.1
    total += int(input("how many nickles?: ")) * 0.05
    total += int(input("how many pennies?: ")) * 0.01
    return total 

def payment_successful(amount_paid, drink_cost):
    """Return True when payment is accepted, or False if money is insufficient."""
    if amount_paid >= drink_cost:
        change = round(amount_paid - drink_cost, 2)
        print(f"You paid ${amount_paid}. Here is ${change} your change.")
        global profit
        profit += drink_cost
        return True
    else:
        print("Sorry that is not enough coins. Money refunded.")
        return False

def prepare_coffee(drink_name, order_ingredients):
    """Deduct the required ingredients from the resources."""
    for item in order_ingredients:
        resources[item] -= order_ingredients[item]
    print(f"Here is your {drink_name}. Enjoy")

prep_on = True

while prep_on:
    choice = input("What would you like to drink? (espresso/Latte/cappuccino): ")
    if choice == "off":
        prep_on = False
    elif choice == "report":
        print(f"Water: {resources['water']}ml")
        print(f"Milk: {resources['milk']}ml")
        print(f"Coffee: {resources['coffee']}g")
        print(f"Money: ${profit}")
    else:
        drink = MENU[choice]
        if resource_availability(drink["ingredients"]):
            payments = payment()
            if payment_successful(payments, drink["cost"]):
              prepare_coffee(choice, drink["ingredients"])  